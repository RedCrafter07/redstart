...
(async () => {
  ...
  if (["--h", "-h", "-help", "--help"].includes(args[0])) {
    return new TextboxBuilder()
      ...
      .addLine(
        `${chalk.redBright("redstart")} ${chalk.yellow(
          "-m --modules"
        )} ${chalk.greenBright("- Get the avilable modules for redstart")}`
      )
      .addLine(
        `${chalk.redBright("redstart")} ${chalk.yellow(
          "-u --usage"
  ...
  if (["-u", "--usage"].includes(args[0])) {
    ...
    if (!existsSync(file))
      return console.log(
        chalk.redBright("[!] Error: " + args[1] + " is not a valid module")
      );
  ...
  if (redstartConfig.dbgprint === "true")
    oldConsoleLog(chalk.yellowBright("[/] Config file parsed successfully!"));
  let exitOnError = redstartConfig.stoponerr === "true";
  ...
  for (const i in moduleObjects) {
    ...
    for (const c of config[modules[i]] || []) {
      try {
        ...
        oldConsoleLog(
          chalk.redBright(
            "[!] Validation Failed: " + modules[i] + " not correctly configured"
          )
        );
        oldConsoleError(e);
        process.exit(1);
  ...
  for (const i in moduleObjects) {
    ...
    for (const c of config[modules[i]] || []) {
      try {
        ...
        oldConsoleLog(
          chalk.redBright("[!] Executing of module " + modules[i] + " failed.")
        );
        oldConsoleLog(
          e.stack && typeof e.stack === "string" ? chalk.red(e.stack) : e
        );
